const sketchProc = (processingInstance) => {
	with (processingInstance) {
		...
		var inhibitingBlocks = [];
		var inhibitingBlockInterval = 10;
		var ticks = 0;
		...
		var draw = function () {
			...
			if (ticks % inhibitingBlockInterval === 0) {
				inhibitingBlocks.push([
					Math.floor((Math.random() * windowWidth) / 10) * 10,
					Math.floor((Math.random() * windowHeight) / 10) * 10,
				]);
			}
			...
			for (var i = 0; i < inhibitingBlocks.length; i++) {
				fill(0, 0, 255);
				rect(inhibitingBlocks[i][0], inhibitingBlocks[i][1], 10, 10);
			}
			...
			for (var i = 0; i < inhibitingBlocks.length; i++) {
				if (
					snakePosition[0] === inhibitingBlocks[i][0] &&
					snakePosition[1] === inhibitingBlocks[i][1]
				) {
					gameOver();
				}
			}
